...
//        })
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if let imageFileURL = imageCatch.objectForKey(restaurant.recordID) as? NSURL {
            ...
            cell.backgroundImage.image = UIImage(data: NSData(contentsOfURL: imageFileURL)!)
        }
        else{
            ...
            if let restaurantRecord = record {
                NSOperationQueue.mainQueue().addOperationWithBlock(){
                    if let imageAsset = restaurantRecord.objectForKey("image")as? CKAsset{
                        cell.backgroundImage.image = UIImage(data: NSData(contentsOfURL: imageAsset.fileURL)!)
